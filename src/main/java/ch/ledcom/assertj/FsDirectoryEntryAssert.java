...
public class FsDirectoryEntryAssert extends AbstractAssert<FsDirectoryEntryAssert, FsDirectoryEntry> {
    ...
    public final synchronized void hasBeenModifiedAt(Date modificationDate, long precision) throws IOException {
        isNotNull();

        if (Math.abs(actual.getLastModified() - modificationDate.getTime()) > precision) {
            String lastModified = DATE_FORMAT.format(new Date(actual.getLastModified()));
